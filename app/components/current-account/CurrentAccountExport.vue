...
const exportEntries = () => {
  ...
  exportToExcel({
    filename: `movimientos_${partyName.replace(/\s+/g, '_')}_${props.currencyCode}`,
    ...
    data: sorted.map(e => ({
      ...
      debit: resolveSide(e.type, props.account?.party_type ?? '') === 'debit' ? Number(e.amount) : null,
      credit: resolveSide(e.type, props.account?.party_type ?? '') === 'credit' ? Number(e.amount) : null,
    }))
  })
}
...
const printStatement = () => {
  ...
  const balanceClass = balance >= 0 ? 'color:#22c55e' : 'color:#ef4444'
  ...
  const html = `<!DOCTYPE html><html><head><title>Estado de Cuenta - ${partyName}</title>
    ...
    <div class="header">
      <div><h2>Estado de Cuenta</h2><p>${partyName} — ${props.currencyCode}</p><p>${props.partyTypeLabel || ''}</p></div>
