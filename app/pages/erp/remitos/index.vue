...
const refresh = () => store.fetchAll({ status: statusFilter.value, category: 'REMITO' })
...
<template>
  <UDashboardPanel>
    <template #header>
      <UDashboardNavbar title="Remitos">
        <template #trailing>
          <UButton icon="i-lucide-file-plus" label="Nuevo remito" :to="`/erp/sales/new?category=REMITO`" />
        </template>
      </UDashboardNavbar>
    </template>
