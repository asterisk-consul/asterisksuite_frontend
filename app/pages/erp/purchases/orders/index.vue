...
const refresh = () => store.fetchAll({ status: statusFilter.value, category: 'ORDER', direction: -1 })
...
<template>
  <UPage class="space-y-4">
    <AppPageHeader title="Órdenes de Compra">
      <template #links>
        <UButton icon="i-lucide-file-plus" label="Nueva OC" :to="`/erp/purchases/orders/new`" />
      </template>
    </AppPageHeader>
        ...
        <UPageCard variant="subtle">
          <UTable :data="documents" :columns="columns" :loading="loading">
            ...
            <template #actions-cell="{ row }">
              <UButton icon="i-lucide-eye" variant="ghost" color="neutral" size="sm" :to="`/erp/purchases/orders/${row.original.id}`" />
            </template>
          </UTable>
        </UPageCard>
      </div>
