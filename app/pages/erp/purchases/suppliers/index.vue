...
const parties = computed(() => allParties.value.filter(p => p.type === 'SUPPLIER'))

onMounted(() => store.fetchAll())

const openEdit = (row: any) => router.push(`/erp/purchases/suppliers/${row.id}/edit`)
const openCreate = () => router.push('/erp/purchases/suppliers/create')
...
const columns = supplierColumns({ onEdit: openEdit, onSortFieldSelect })
  ...
  <UPage class="space-y-4">
    <AppPageHeader title="Proveedores" description="Gestión de proveedores del sistema">
      <template #links>
        ...
        <UButton label="Nuevo proveedor" icon="i-heroicons-plus" color="primary" @click="openCreate" />
      </template>
    </AppPageHeader>
