...
const refresh = () => store.fetchAll({ status: statusFilter.value, documentTypeId: undefined, category: 'REMITO', direction: -1 })
...
<template>
  <UPage class="space-y-4">
    <AppPageHeader title="Remitos de Compra">
      <template #links>
        <UButton icon="i-lucide-file-plus" label="Nuevo remito" :to="`/erp/purchases/remitos/new`" />
      </template>
    </AppPageHeader>
        ...
        <UPageCard variant="subtle">
          <UTable :data="documents" :columns="columns" :loading="loading">
            ...
            <template #actions-cell="{ row }">
              <UButton icon="i-lucide-eye" variant="ghost" color="neutral" size="sm" :to="`/erp/purchases/remitos/${row.original.id}`" />
            </template>
          </UTable>
        </UPageCard>
      </div>
