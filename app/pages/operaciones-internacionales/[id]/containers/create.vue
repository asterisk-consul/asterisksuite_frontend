...
const onSubmit = async () => {
  try {
    await createContainer(operationId, form.value)
  ...
  <UPage class="space-y-6 max-w-3xl">
    ...
    <UForm :state="form" @submit="onSubmit" class="space-y-6">
      ...
      <div class="flex justify-end gap-2">
        <UButton label="Cancelar" variant="ghost" :to="`/operaciones-internacionales/${operationId}`" />
        <UButton label="Crear contenedor" color="primary" type="submit" :loading="loading" />
      </div>
    </UForm>
  </UPage>
</template>
