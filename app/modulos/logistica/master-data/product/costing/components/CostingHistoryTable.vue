...
const columns: TableColumn<CostHistoryRow | CostHistoryBreakdown>[] = [
  {
    ...
    cell: ({ row }) => {
      ...
      return h('div', { class: 'flex items-center', style: { paddingLeft: `${row.depth * 1.5}rem` } }, [
        h(UButton, {
          color: 'neutral',
          variant: 'outline',
          size: 'xs',
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      if ('component_product_id' in original) {
        return h('div', { class: 'flex flex-col gap-0.5', style: { paddingLeft: `${row.depth * 1.5}rem` } }, [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const original = row.original as any
      if ('component_product_id' in original) {
        return h('span', { class: 'text-sm text-muted' }, `× ${original.quantity}`)
      }
      return h(UBadge, {
        label: original.cost_source,
        color: COST_SOURCE_COLORS[original.cost_source] ?? 'gray',
        variant: 'soft',
        size: 'xs'
      })
    }
