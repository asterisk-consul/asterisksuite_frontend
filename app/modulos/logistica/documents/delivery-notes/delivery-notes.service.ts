...
export const useDeliveryNotesService = () => {
  const findAll = (query?: QueryDeliveryNoteInput) =>
    $fetch<DeliveryNote[]>('/api/backend/delivery-notes', {
      query
    })

  const findOne = (id: string) =>
    $fetch<DeliveryNote>(`/api/backend/delivery-notes/${id}`)
  ...
  const remove = (id: string) =>
    $fetch<void>(`/api/backend/delivery-notes/${id}`, {
      method: 'DELETE'
    })

  const confirm = (id: string) =>
    $fetch<DeliveryNote>(`/api/backend/delivery-notes/${id}/confirm`, {
      method: 'POST'
    })
